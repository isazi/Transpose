...
DB_CONN = pymysql.connect(host=config.HOST, port=config.PORT, user=config.USER, passwd=config.PASS, db=config.TRANSPOSE_DB)
...
if COMMAND == "create":
    ...
    if len(sys.argv) != 5:
        print("Usage: " + sys.argv[0] + " tune <table> <operator> <N>")
        QUEUE.close()
        DB_CONN.close()
        sys.exit(1)
    try:
